 ...
 */
function corporatex_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'corporatex_form_submit';
}


function corporatex_form_submit($form, &$form_state) {
  // Import database corporatex file.
  $corporatex_file = dirname(__FILE__) . '/corporatex.sql';
  $success = import_corporatex($corporatex_file);
...
/**
 * Imports a database corporatex file.
 *
 * @see demo_reset().
 */
function import_corporatex($filename) {

  global $databases;
  $db_prefix = $databases["default"]["default"]["prefix"];
  
  // Open corporatex file.
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open corporatex file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
  ...
  // Load data from corporatex file.
  $success = TRUE;
  $query = '';
  $new_line = TRUE;
